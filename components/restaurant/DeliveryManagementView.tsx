    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-surface rounded-lg shadow-md p-4 border border-border">
                <h2 className="text-lg font-bold text-copy-rich mb-4 border-b pb-2 border-border">Ready for Driver ({ordersReadyForDriver.length})</h2>
                <div className="space-y-4 max-h-[70vh] overflow-y-auto">
                    {ordersReadyForDriver.map(order => (
                        <div key={order.id} className="bg-background p-3 rounded-lg border border-border">
                            <p className="font-bold text-copy">Order {order.id.slice(-6)} for {order.orderName}</p>
                            <p className="text-sm text-copy-light mt-1">{order.deliveryAddress}</p>
                            <button 
            ...
            <div className="bg-surface rounded-lg shadow-md p-4 border border-border">
                <h2 className="text-lg font-bold text-copy-rich mb-4 border-b pb-2 border-border">Out for Delivery ({ordersOutForDelivery.length})</h2>
                 <div className="space-y-4 max-h-[70vh] overflow-y-auto">
                    {ordersOutForDelivery.map(order => (
                        <div key={order.id} className="bg-yellow-500/10 p-3 rounded-lg border border-yellow-500/30">
                            <p className="font-bold text-copy">Order {order.id.slice(-6)} for {order.orderName}</p>
                            <p className="text-sm text-copy-light mt-1">{order.deliveryAddress}</p>
                            <button 
            ...
            {/* Column 3: Delivered */}
            <div className="bg-surface rounded-lg shadow-md p-4 border border-border">
                <h2 className="text-lg font-bold text-copy-rich mb-4 border-b pb-2 border-border">Delivered ({ordersDelivered.length})</h2>
                 <div className="space-y-4 max-h-[70vh] overflow-y-auto">
                    {ordersDelivered.map(order => (
                        <div key={order.id} className="bg-green-500/10 p-3 rounded-lg border border-green-500/20 opacity-70">
                            <p className="font-bold text-copy">Order {order.id.slice(-6)} for {order.orderName}</p>
                            <p className="text-sm text-copy-light mt-1">{order.deliveryAddress}</p>
                        </div>
                    ))}
