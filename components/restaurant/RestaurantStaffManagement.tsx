    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-lg p-6">
                <h2 className="text-2xl font-bold text-brand-charcoal mb-4">Add Staff Member</h2>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                     ...
                     <div>
                        ...
                        <select name="role" value={formData.role} onChange={handleChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-brand-gold focus:ring-brand-gold sm:text-sm">
                    ...
                    <div className="flex justify-end gap-4 mt-6">
                        ...
                        <button type="submit" className="px-4 py-2 text-sm font-medium text-white bg-brand-charcoal rounded-md hover:bg-opacity-90">Add Staff</button>
                    </div>
                </form>
            </div>
        </div>
...
const EditStaffModal: React.FC<{
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-lg p-6">
                <h2 className="text-2xl font-bold text-brand-charcoal mb-4">Reset PIN for {staffMember.name}</h2>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-gray-700">New PIN (4-digits)</label>
                        <input type="password" name="pin" value={pin} onChange={(e) => setPin(e.target.value)} maxLength={4} pattern="\d{4}" title="PIN must be 4 digits" className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-brand-gold focus:ring-brand-gold sm:text-sm" required />
                    </div>
                    <div className="flex justify-end gap-4 mt-6">
                        <button type="button" onClick={onClose} className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-md hover:bg-gray-200">Cancel</button>
                        <button type="submit" className="px-4 py-2 text-sm font-medium text-white bg-brand-charcoal rounded-md hover:bg-opacity-90">Reset PIN</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                 <h3 className="text-xl font-bold text-brand-charcoal">Your Team</h3>
                <button 
                    onClick={() => setIsCreateModalOpen(true)} 
                    className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-brand-charcoal rounded-md hover:bg-opacity-90"
                >
                ...
                <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {staff.length > 0 ? staff.map(s => (
                            <tr key={s.id} className={s.status === 'suspended' ? 'bg-gray-50' : ''}>
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <span className={`font-medium ${s.status === 'suspended' ? 'text-gray-400' : 'text-gray-900'}`}>{s.name}</span>
                                </td>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-center text-sm font-medium space-x-2">
                                    <button onClick={() => setEditingStaff(s)} className="text-gray-400 hover:text-brand-gold p-1" title="Reset PIN">
                                        <EditIcon className="w-5 h-5" />
                                    </button>
                                    {s.status === 'active' ? (
                                        <button onClick={() => setActionToConfirm({ type: 'suspend', data: s })} className="text-gray-400 hover:text-yellow-600 p-1" title="Suspend User">
                                            <UserXIcon className="w-5 h-5" />
                                        </button>
                                    ) : (
                                        <button onClick={() => setActionToConfirm({ type: 'activate', data: s })} className="text-gray-400 hover:text-green-600 p-1" title="Activate User">
                                            <UserCheckIcon className="w-5 h-5" />
                                        </button>
                                    )}
                                    <button onClick={() => setActionToConfirm({ type: 'delete', data: s })} className="text-gray-400 hover:text-red-600 p-1" title="Delete User">
                            ...
                            <tr>
                                <td colSpan={4} className="text-center py-10 text-gray-500">No staff have been added yet.</td>
                            </tr>
                        )}
                    </tbody>
                </table>
